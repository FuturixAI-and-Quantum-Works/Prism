...
type SystemWorkflowSeed = Pick<
  typeof workflows.$inferInsert,
  "stableKey" | "title" | "type" | "promptMd" | "userId" | "isSystem"
>;

export const SYSTEM_WORKFLOWS = [
  {
    stableKey: "builtin-cp-checklist",
  ...
  {
    stableKey: "builtin-credit-summary",
...
] satisfies readonly SystemWorkflowSeed[];

export function validateSystemWorkflows(): void {
  const keys = new Set<string>();
  for (const workflow of SYSTEM_WORKFLOWS) {
    if (!workflow.stableKey || !/^builtin-[a-z0-9-]+$/.test(workflow.stableKey)) {
      throw new Error(`Invalid workflow stable key: ${workflow.stableKey ?? "missing"}`);
    }
    if (keys.has(workflow.stableKey)) {
      throw new Error(`Duplicate workflow stable key: ${workflow.stableKey}`);
    }
    keys.add(workflow.stableKey);
    if (workflow.userId !== null || workflow.isSystem !== true || !workflow.promptMd?.trim()) {
      throw new Error(`Invalid system workflow: ${workflow.stableKey}`);
    }
  }
}

export async function seedWorkflows(): Promise<void> {
  validateSystemWorkflows();
  await db.transaction(async (transaction) => {
    for (const workflow of SYSTEM_WORKFLOWS) {
      await transaction
        ...
        .values(workflow)
        .onConflictDoUpdate({
          target: workflows.stableKey,
          set: {
            title: workflow.title,
            type: workflow.type,
            promptMd: workflow.promptMd,
            userId: null,
            isSystem: true,
            updatedAt: new Date(),
          },
...
if (entrypoint && import.meta.url === pathToFileURL(path.resolve(entrypoint)).href) {
  if (process.argv.includes("--dry-run")) {
    validateSystemWorkflows();
    console.info(`Validated ${SYSTEM_WORKFLOWS.length} system workflows.`);
  } else {
    withScriptDatabase(process.env, seedWorkflows)
      .then(() => {
        console.info(`Seeded ${SYSTEM_WORKFLOWS.length} system workflows.`);
      })
      .catch((error: unknown) => {
